' A form-style grid: labels in column 0, fields in column 1, a button
' spanning both columns on its own row.
...
app = HApplicationCreate("application/x-vnd.EbHaiku-GridLayoutExample")

DIM w AS HWindow
w = HWindowCreate(100, 100, 420, 260, "eb-haiku grid layout example", H_QUIT_ON_WINDOW_CLOSE)
