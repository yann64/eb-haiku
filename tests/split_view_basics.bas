' Step 5: BSplitView - two resizable panes with a draggable splitter
' between them. Dragging itself isn't automatable over SSH (same real-
' hardware-input limitation as custom MouseDown/KeyDown testing) - what
' is checkable is that the split view renders correctly with both
' children visible, verified via an external screenshot (see
...
app = HApplicationCreate("application/x-vnd.EbHaiku-SplitViewTest")

DIM w AS HWindow
w = HWindowCreate(100, 100, 420, 260, "eb-haiku split view test", H_QUIT_ON_WINDOW_CLOSE)
